...
_CASE0_LAUNCH_V4_DEFAULT = "LAUNCH(16384, 128, 128, 256, 2, true,  true, 6)"
CASE0_VARIANT = os.environ.get("AISP_NVFP4_CASE0_VARIANT", "v3b").strip().lower()


def _make_v4_src_with_case0(module_name: str, case0_launch: str) -> str:
    src = CUDA_SRC_V4.replace(_CASE0_LAUNCH_V4_DEFAULT, case0_launch)
    return src.replace("TORCH_LIBRARY(my_module_v4, m)", f"TORCH_LIBRARY({module_name}, m)")


_CASE0_V4_VARIANTS = {
    "v4_default": ("my_module_v4", _CASE0_LAUNCH_V4_DEFAULT),
    "v4_n64_s1": ("my_module_v4_case0_n64_s1", "LAUNCH(16384, 128,  64, 256, 1, true, false, 8)"),
    "v4_n64_split2": ("my_module_v4_case0_n64_split2", "LAUNCH(16384, 128,  64, 256, 2, true, false, 8)"),
    "v4_n128_s1": ("my_module_v4_case0_n128_s1", "LAUNCH(16384, 128, 128, 256, 1, true,  true, 6)"),
    "v4_m64_n128_split2_s8": ("my_module_v4_case0_m64_n128_split2_s8", "LAUNCH(16384,  64, 128, 256, 2, true,  true, 8)"),
    "v4_m64_n128_split1_s8": ("my_module_v4_case0_m64_n128_split1_s8", "LAUNCH(16384,  64, 128, 256, 1, true,  true, 8)"),
    "v4_m64_n64_split2_s8": ("my_module_v4_case0_m64_n64_split2_s8", "LAUNCH(16384,  64,  64, 256, 2, true, false, 8)"),
    "v4_m64_n64_split1_s8": ("my_module_v4_case0_m64_n64_split1_s8", "LAUNCH(16384,  64,  64, 256, 1, true, false, 8)"),
    "v4_m64_n128_bk512_s3": ("my_module_v4_case0_m64_n128_bk512_s3", "LAUNCH(16384,  64, 128, 512, 1, true,  true, 3)"),
}

_selected_case0_module = None
_selected_case0_launch = None
if CASE0_VARIANT in _CASE0_V4_VARIANTS:
    _selected_case0_module, _selected_case0_launch = _CASE0_V4_VARIANTS[CASE0_VARIANT]

_cuda_sources = [
    CUDA_SRC_COMMON + _make_v4_src_with_case0("my_module_v4", _CASE0_LAUNCH_V4_DEFAULT),
    CUDA_SRC_COMMON + CUDA_SRC_V3B,
]
...
def custom_kernel(data: input_t) -> output_t:
    global start
    global CASE0_PHASE_ANCHOR
    k = data[0].shape[1] * 2
    if k == 16384:
        if CASE0_VARIANT == "v3b":
            return gemm_v3b(data[0], data[1], data[4], data[5], data[6])
        case0_anchor = int(data[0].data_ptr())
        zero_case0_slab = False
        if ALLOC_CALL_MOD == 0 and CASE0_PHASE_ANCHOR != case0_anchor:
            CASE0_PHASE_ANCHOR = case0_anchor
            start = 0
            zero_case0_slab = True
        if gemm_case0_v4_alt is not None:
            return gemm_case0_v4_alt(data[0], data[1], data[4], data[5], data[6], allocate(data[6], zero_slab=zero_case0_slab))
        return gemm_v4(data[0], data[1], data[4], data[5], data[6], allocate(data[6], zero_slab=zero_case0_slab))

    if k == 7168:
        case1_buf = allocate(data[6], zero_slab=True)
        return gemm_v4(data[0], data[1], data[4], data[5], data[6], case1_buf)
